...
func _ready() -> void:
	...
	# Show tutorial on first play (suppressed during DTS_SHOT captures — the
	# modal overlay dims the whole frame)
	if GameManager.swamp_states[0]["gallons_drained"] <= 0.001 and OS.get_environment("DTS_SHOT") == "":
...
func _build_shop() -> void:
	...
	if not V3_SIGNAGE:
		...
		# (v3 props round 3). Moot while V3_SIGNAGE is on (signage.gd's own
		# SELL plank replaces this), kept in sync in case that ever flips.
		sell_lbl.z_index = 6
		add_child(sell_lbl)
